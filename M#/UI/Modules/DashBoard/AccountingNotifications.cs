...
namespace Modules
{
    class AccountingNotificationsList : BaseListModule<Domain.AccountingNotification>
    {
        public AccountingNotificationsList()
        {
            HeaderText("Accounting Notifications")
                  .SortingStatement("item.NotificationDate DESC");
            ...
            .MemoryFilterCode(@"if(info.CustomerName.HasValue())
                                 { 
                                        result = result.Where(p => p.Shipment.PrimaryContact.Name.ToUpper().Contains(info.CustomerName.ToUpper()));
                                }");
            ...
            Column(x => x.Shipment)
              ...
              .DisplayExpression("@item.Shipment.PrimaryContact.Name");

            Column(x => x.Error)
                .LabelText("Task");

            Column(x => x.NotificationDate)
