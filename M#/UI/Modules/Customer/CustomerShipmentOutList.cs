...
namespace Modules
{
    class CustomerShipmentOutList : BaseListModule<Domain.Shipment>
    {
        public CustomerShipmentOutList()
        {
            RootCssClass("mfa");
            this.AddDependency<Domain.IMFAService>();
            this.AddDependency<Domain.ISmsService>();
            HeaderText("Shipments Out of UK")
                .PageSize(10)
            ...
            Search(x => x.Date)
                .Label("Date created")
                .VisibleIf("!info.TrackingNumber.HasValue() && !info.MyReferenceForCPInvoice.HasValue()").MemoryFilterCode("//remove from here");

            Search(x => x.ExpectedDate)
                .Label("Expected date of arrival/departure")
                .VisibleIf("!info.TrackingNumber.HasValue() && !info.MyReferenceForCPInvoice.HasValue()").MemoryFilterCode("//remove from here");

            Search(x => x.MyReferenceForCPInvoice).Label("Customer Reference").MemoryFilterCode("//remove from here").MemoryFilterCode("//remove from here");
            ...
            {
                x.CSharp("await SearchUrlCookie.ClearCookieHistory(isNcts: false, isIntoUk: false);");
            ...
            Box("ShipmentHeader", BoxTemplate.HeaderBox)
                ...
                .Visibility("await SearchUrlCookie.IsActiveShipmentSearch(isNcts: false, isIntoUk: false)");

            Box("ConsignmentHeader", BoxTemplate.HeaderBox)
               .Text($"Consignment Level Search Filter: Active {consigmentButton.Ref}")
               .Visibility("await SearchUrlCookie.IsActiveConsignmentSearch(isNcts: false, isIntoUk: false)");
            ...
            Column(x => x.Date).DisplayFormat("{0: d}");
            Column(x => x.ExpectedDate).LabelText("Expected date of arrival/departure");
            ...
            Column(x => x.Progress).DisplayExpression("@item.Progress.AdminDisplay");

            LinkColumn("Consignments").HeaderText("Consignments").Text(@"@await item.Consignments.Count()")
              .OnClick(x =>
              {
                  x.Go<Share.Consignments.ConsignmentsPage>().Send("shipment", "item.ID").SendReturnUrl();
              });

            Column(x => x.IsWeightsMismatch).LabelText("Weights mismatch");
            ...
            ButtonColumn("Print")
                ...
                .OnClick(x =>
                {
                    x.Go<Share.Shipment.ShipmentView.PrintPage>().Target(OpenIn.NewBrowserWindow).Send("shipment", "item.ID");
                });
            ...
            ButtonColumn("Raise Support Ticket").HeaderText("Actions")
             .GridColumnCssClass("actions-merge")
            ...
            .OnClick(x => x.PopUp<Customer.ConsignmentOutSearchEnterPage>()
                .Send("item", @"Request.Query[""ID""]")
                .SendReturnUrl());

            Button("New Shipment").OnClick(x => x.Go<Share.Shipment.ShipmentEnterPage>().SendReturnUrl())
                .Icon(FA.Plus);


            //search logic
            OnPostBound("set Cookie for url").Code("await SearchUrlCookie.CreateCookieHistory(info.Date, info.DateMax, info.ExpectedDate, info.ExpectedDateMax,isNcts: false, isIntoUk: false);");

            OnControllerClassCode("set time").Code(@"static void DateSet(vm.CustomerShipmentOutList info,ConsignmentSearch consigmentSearch = null )
            {       
            ...
            OnControllerClassCode("filter logic").Code(@"private async Task Filter(vm.CustomerShipmentOutList info)
            {
