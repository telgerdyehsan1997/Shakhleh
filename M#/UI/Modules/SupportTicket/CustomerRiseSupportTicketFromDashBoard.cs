...
namespace Modules
{
    class CustomerRiseSupportTicketFromDashBoard : FormModule<Domain.SupportTicket>
    {
        public CustomerRiseSupportTicketFromDashBoard()
        {
            ...
            .OnClick(x =>
            {
                ...
                x.CSharp(@"if(info.IsTrackingNumber)
                        return Notify(info.TrackingNumber + "" is invalid tracking number."", ""error"");
                ");
            ...
            {  
                ...
                info.Item.Company = Context.Current.User().ExtractUser<CompanyUser>().Company;
                info.Shipment = await Database.Of<Shipment>().Where(x => x.TrackingNumber == info.TrackingNumber && x.Company == info.Item.Company).FirstOrDefault();
