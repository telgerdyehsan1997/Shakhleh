...
{
    partial class ShipmentsOutUKController
    {
        Olive.DatabaseFilters<Shipment> DatabaseFilters;
        public async Task<IEnumerable<Shipment>> FilterShipments(vm.CustomerShipmentOutList info, Company company)
        {
            DatabaseFilters = new DatabaseFilters<Shipment>();
            DatabaseFilters.Add(x => x.CompanyId == company);
            DatabaseFilters.Add(x => x.TypeId == ShipmentType.OutOfUk);
