...
{
    partial class AdminShipmentController
    {
        Olive.DatabaseFilters<Shipment> DatabaseFilters;
        public async Task<IEnumerable<Shipment>> FilterShipments(vm.ShipmentList info)
        {
            ...
            if (info.Type != null)
                DatabaseFilters.Add(s => s.TypeId == info.Type);
            ...
            if (info.Company != null)
                DatabaseFilters.Add(x => x.CompanyId == info.Company);
