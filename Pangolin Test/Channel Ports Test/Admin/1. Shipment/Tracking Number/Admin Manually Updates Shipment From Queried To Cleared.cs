...
{
    [TestClass]
    public class AdminManuallyUpdatesShipmentFromQueriedToCleared : UITest
    {
        [PangolinTestMethod]
        public override void RunTest()
        {
            ...
            AtRow(consignment.ConsignmentNumber).Column("Progress").Click("Queried");
            ExpectHeader("Progress History");
            Set("Progress").To("Cleared");
            ClickButton("Save");

            //Asserts that Progress has been updated
            AtRow(consignment.ConsignmentNumber).Column("Progress").Expect("Cleared");
        }
    }
}